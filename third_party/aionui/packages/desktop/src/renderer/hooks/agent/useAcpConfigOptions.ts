...
import {
  getManagedKimiThinkingLabel,
  isManagedKimiModelCatalog,
  toManagedKimiThinkingModel,
...
export function deriveSelectOption(
  ...
  const isManagedKimiModel =
    category === 'model' && isManagedKimiModelCatalog(option.options.map((choice) => choice.value));
  if (isManagedKimiModel) {
    return {
      id: option.id,
      category,
      currentValue: toManagedKimiThinkingModel(getOptionCurrentValue(option)),
      options: option.options
        .filter((choice) => getManagedKimiThinkingLabel(choice.value))
        .map((choice) => ({
          value: choice.value,
          label: getManagedKimiThinkingLabel(choice.value)!,
...
export function confirmKimiLegacyModelAfterTimeout(
  options: AcpConfigOptionDto[] | null,
  optionId: string,
  requestedValue: string,
  errorKind: AcpConfigSetErrorKind
): AcpConfigOptionDto[] | null {
  if (!options || errorKind !== 'confirmation_timeout' || !getManagedKimiThinkingLabel(requestedValue)) return null;
  const model = options.find(
    (option) => option.id === optionId && (option.category === 'model' || option.id === 'model')
  );
  if (!model || !model.options.some((choice) => choice.value === requestedValue)) return null;
  return options.map((option) => (option === model ? { ...option, current_value: requestedValue } : option));
}
    ...
    async (optionId: string, value: string) => {
      ...
      try {
        ...
        try {
          ...
          const confirmed = confirmKimiLegacyModelAfterTimeout(
            optionsRef.current,
            optionId,
            value,
            classifyConfigSetError(error)
          );
          if (!confirmed) throw error;
          replaceSnapshot(confirmed);
          return confirmed;
        }
