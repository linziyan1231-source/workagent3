...
const CODEX_REASONING_FALLBACK: AgentRuntimeSelectState = {
  id: 'reasoning_effort',
...
const CreateTaskDialog: React.FC<CreateTaskDialogProps> = ({
  ...
  const acpCachedModelInfo = useMemo<AcpModelInfo | null>(() => {
    ...
    return buildAgentRuntimeModelInfo(selectedAgentRuntime) ?? buildAssistantModelInfo(selectedAssistantModels);
  }, [resolvedBackend, selectedAgentRuntime, selectedAssistantModels]);

  const acpThoughtLevel = useMemo<AgentRuntimeSelectState | null>(() => {
    if (!resolvedBackend || resolvedBackend === 'gemini' || resolvedBackend === 'aionrs') return null;
    return (
      buildAgentRuntimeThoughtLevel(selectedAgentRuntime) ??
      (resolvedBackend === 'codex' ? CODEX_REASONING_FALLBACK : null)
    );
  }, [resolvedBackend, selectedAgentRuntime]);
  ...
  const showReasoningSelector = Boolean(acpThoughtLevel?.options.length);
  const advancedFieldCount = Number(showModelSelector) + Number(showReasoningSelector) + 1;
            ...
            <div className='mt-16px'>
              ...
              {advancedOpen && (
                <div className='mt-12px grid gap-x-16px gap-y-16px md:grid-cols-2'>
                  ...
                  {showReasoningSelector && acpThoughtLevel && (
                    <div className='min-w-0'>
                      <label className='mb-8px block text-14px font-medium text-t-primary'>
                        {t('acp.config.reasoning_effort')}
                      </label>
                      <Select
                        data-testid='cron-reasoning-select'
                        value={config_options?.[acpThoughtLevel.id]}
                        placeholder={acpThoughtLevel.currentValue}
                        onChange={(value: string) => {
                          setConfigOptions((current) => {
                            const next = { ...current };
                            delete next.reasoning_effort;
                            delete next.thought_level;
                            next[acpThoughtLevel.id] = value;
                            return next;
                          });
                        }}
                      >
                        {acpThoughtLevel.options.map((option) => (
