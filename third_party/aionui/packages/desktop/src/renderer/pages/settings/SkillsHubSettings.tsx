...
type LocalizedSkillInfo = Pick<SkillInfo, 'name' | 'description'>;
type SkillCatalog = Record<string, LocalizedSkillInfo>;
...
const RefreshIconButton: React.FC<RefreshIconButtonProps> = ({ label, loading, onClick, testId }) => (
  <button
    type='button'
    data-testid={testId}
...
const SkillsHubSettings: React.FC<SkillsHubSettingsProps> = ({ withWrapper = true }) => {
  ...
  const canUseMarket = !isElectronDesktop() && Boolean(user?.collaboration_capable) && !user?.admin;
  ...
  const localizedBuiltinSkills = useMemo<SkillCatalog>(() => {
    const language = i18n.resolvedLanguage ?? i18n.language;
    if (!language?.startsWith('zh')) return {};
    const resources = i18n.getResourceBundle(language, 'translation') as
      | { settings?: { skillsHub?: { builtinSkillCatalog?: SkillCatalog } } }
      | undefined;
    return resources?.settings?.skillsHub?.builtinSkillCatalog ?? {};
  }, [i18n, i18n.language, i18n.resolvedLanguage]);

  const getSkillDisplay = useCallback(
    (skill: SkillInfo): LocalizedSkillInfo => {
      if (skill.source !== 'builtin') return skill;
      return localizedBuiltinSkills[skill.name] ?? skill;
    },
  ...
  const filteredSkills = useMemo(() => {
    ...
    return mySkills.filter((skill) => {
      ...
      return [skill.name, skill.description, display.name, display.description].some((value) =>
        value?.toLowerCase().includes(lowerQuery)
      );
    });
  }, [getSkillDisplay, mySkills, search_query]);
  ...
  const fetchMarket = useCallback(async () => {
    ...
    try {
      const result = await ipcBridge.portal.listSkillMarket.invoke();
  ...
  const publishSkill = useCallback(
    async (skillName: string) => {
      try {
        await ipcBridge.portal.publishSkill.invoke({ skill_name: skillName });
        Message.success(t('settings.skillsHub.marketPublished'));
        await fetchMarket();
      } catch {
        Message.error(t('settings.skillsHub.marketPublishFailed'));
      }
    },
    [fetchMarket, t]
  );
    ...
    (entry: PortalSkillMarketEntry) => {
      ...
      Modal.confirm({
        ...
        onOk: async () => {
          ...
          Message.success(t('settings.skillsHub.marketInstalled'));
          await fetchData();
        },
      });
    },
  ...
  const deleteMarketSkill = useCallback(
    (entry: PortalSkillMarketEntry) => {
      Modal.confirm({
        title: t('settings.skillsHub.marketDeleteTitle'),
        content: t('settings.skillsHub.marketDeleteWarning', { name: entry.name }),
        okButtonProps: { status: 'danger' },
        onOk: async () => {
          await ipcBridge.portal.deleteMarketSkill.invoke({ id: entry.id });
  ...
  const mainContent = isImportHistoryView ? (
    ...
    <div className='flex flex-col h-full w-full'>
      <div className='space-y-16px pb-24px'>
        ...
        >
          <summary className='cursor-pointer text-16px md:text-18px text-t-primary font-bold tracking-tight mb-20px'>
            {t('settings.skillsHub.mySkillsTitle', { defaultValue: 'My Skills' })}
            <span className='bg-[rgba(var(--primary-6),0.08)] text-primary-6 text-12px px-10px py-2px rd-[100px] font-medium ml-8px'>
              {mySkills.length}
            </span>
          </summary>
                  ...
                  >
                    ...
                    <div className='flex-1 min-w-0 flex flex-col justify-center gap-6px'>
                      <div className='flex items-center gap-10px flex-wrap'>
                        <h3 className='text-14px font-semibold text-t-primary/90 truncate m-0'>{display.name}</h3>
                        {skill.source === 'custom' ? (
        ...
        {builtinAutoSkills.length > 0 && (
          <details
            ...
            className='px-[16px] md:px-[32px] py-32px bg-base rd-16px md:rd-24px shadow-sm border border-b-base relative overflow-hidden transition-all'
          >
            <summary className='cursor-pointer flex items-center gap-10px mb-24px'>
        ...
        {canUseMarket && (
          <div
            data-testid='skill-market-section'
            className='px-[16px] md:px-[32px] py-32px bg-base rd-16px md:rd-24px shadow-sm border border-b-base'
          >
            <div className='flex items-center justify-between gap-12px mb-20px'>
              <div>
                <h2 className='text-18px text-t-primary font-bold m-0'>{t('settings.skillsHub.marketTitle')}</h2>
                <p className='text-13px text-t-secondary mt-6px mb-0'>{t('settings.skillsHub.marketDescription')}</p>
              </div>
              <RefreshIconButton
                testId='btn-refresh-skill-market'
            ...
            {marketSkills.length === 0 ? (
              <div className='text-13px text-t-secondary py-24px'>
                {marketLoading ? t('common.loading') : t('settings.skillsHub.marketEmpty')}
              </div>
            ) : (
              <div className='flex flex-col gap-8px'>
