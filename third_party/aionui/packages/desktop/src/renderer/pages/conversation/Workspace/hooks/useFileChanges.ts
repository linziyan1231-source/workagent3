...
export function useFileChanges({ workspace }: UseFileChangesParams): UseFileChangesReturn {
  const supportsSnapshots = Boolean(workspace) && !workspace.startsWith('shared://');
  ...
  const stageFile = useCallback(
    ...
    [supportsSnapshots, workspace, silentRefresh]
  );
