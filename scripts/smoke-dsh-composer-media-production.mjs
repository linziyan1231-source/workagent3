...
const out = resolve(process.env.WORKAGENT_COLLAB_EVIDENCE_DIR || ".cache/composer-media/production");
const candidate = resolve(process.env.WORKAGENT_COLLAB_CLIENT_DIR || ".cache/composer-media/client");
...
try {
  ...
  const composerWidth=(await page.locator('.workagent-collab-composer').boundingBox()).width;
  const areaWidth=(await page.locator('.workagent-collab-workspace').boundingBox()).width;
  assert(Math.abs(composerWidth-(Math.min(areaWidth,1080)-80))<1,'shared composer must use personal task width');
  const centers=await page.locator('.workagent-top-notifications, .workagent-collab-header-icon').evaluateAll(rows=>rows.map(row=>{const b=row.getBoundingClientRect();return b.y+b.height/2;}));
  assert(Math.max(...centers)-Math.min(...centers)<1,'header controls must align');
  report.checks.push('shared/personal content width rule and aligned notification button');
