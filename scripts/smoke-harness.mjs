...
try {
  ...
  while (Date.now() < deadline) {
    if (child.exitCode !== null) throw new Error(`Harness exited with ${child.exitCode}: ${diagnostics}`);
    try {
      ...
      if (response.ok) {
        ...
        if (body.status !== 'healthy') throw new Error('unexpected health response');
        process.stdout.write(`workagent Harness profile healthy on loopback port ${port}\n`);
        break;
      }
    } catch {
  ...
  if (Date.now() >= deadline) throw new Error(`Harness health timeout: ${diagnostics}`);
} finally {
  child.kill();
}
