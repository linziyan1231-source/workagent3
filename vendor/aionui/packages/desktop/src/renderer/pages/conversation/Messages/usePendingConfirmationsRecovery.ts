...
export function usePendingConfirmationsRecovery(conversation_id: string, options?: { disabled?: boolean }) {
  const updateMessageList = useUpdateMessageList();

  useEffect(() => {
    if (!conversation_id || options?.disabled) return;
    let cancelled = false;
  ...
  }, [conversation_id, options?.disabled, updateMessageList]);
}
