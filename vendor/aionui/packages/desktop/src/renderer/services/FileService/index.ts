...
import { UPLOAD_ABORTED_ERROR, uploadFileResumably } from './resumableUpload';

export { UPLOAD_ABORTED_ERROR } from './resumableUpload';

export interface UploadFileOptions {
  /** Cancel the active request while retaining the backend session for a later resume. */
  signal?: AbortSignal;
}

/**
 * Upload a file to the backend in resumable, sequential chunks.
 *
 ...
 *
 * The public signature is kept stable for existing drag, paste, and attach
 * callers. Internally the upload is initialized, resumed from the server's
 * confirmed offset, and completed after all chunks are accepted.
 *
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      if (!file_path) {
        ...
        try {
          ...
          // Let every UI entry point surface its existing upload-failed toast.
          // Silently continuing here made the progress row flash and disappear
          // without telling the user why no attachment was added.
          throw error;
        } finally {
          tracker.finish();
        }
