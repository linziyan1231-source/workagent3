...
export const useAutoPreviewOfficeFiles = (
  ...
  const candidateWorkspace = conversation?.workspace?.trim() ? conversation.workspace : undefined;
  const workspace = candidateWorkspace?.startsWith('shared://') ? undefined : candidateWorkspace;
