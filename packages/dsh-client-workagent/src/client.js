  ...
  factory: (require) => {
    ...
    function ComposerForm({ children, className, ...props }) {
      ...
      React.useLayoutEffect(() => {
        ...
        const measure = () => container.style.setProperty("--workagent-composer-height", `${form.getBoundingClientRect().height}px`);
      ...
      return h("form", { ...props, ref: formRef, className: `${className} workagent-compact-composer`, "data-options-open": expanded },
        children,
        h("button", { type: "button", className: "workagent-composer-settings", "aria-label": "模型与权限设置", "aria-expanded": expanded, onClick: () => setExpanded(!expanded) }, h(Icon, { name: "settings", size: 18 })),
      );
    }
    ...
    const iconPaths = {
      settings: ["M8 12h10m8 0h14M8 24h22m8 0h2M8 36h4m8 0h20M18 7v10m12 2v10M12 31v10"],
      assistants: [
    ...
    function QuotaPanel() {
      const [state, refresh] = useResource("/api/quota/dollars", value => value.budgets || []);
      React.useEffect(() => { const timer = setInterval(refresh, 5000); return () => clearInterval(timer); }, [refresh]);
      const remaining = (used, limit) => `${limit > 0 ? Math.round(Math.max(0, Math.min(1, 1 - used / limit)) * 100) : 0}%`;
      const meter = (label, used, limit) => { const value = remaining(used, limit); return h("div", { className: "workagent-quota-row" }, h("span", null, label), h("span", null, value), h("div", { className: "workagent-quota-track", role: "progressbar", "aria-label": label, "aria-valuemin": 0, "aria-valuemax": 100, "aria-valuenow": parseInt(value, 10) }, h("span", { style: { width: value } }))); };
      return h("aside", { className: "workagent-quota-panel", "aria-label": "使用额度" },
        h("strong", null, "剩余额度"),
        h(Status, { state }),
        ...state.rows.map(b => h("div", { className: "workagent-dollar-quota", key: b.pool },
        ...
        h("span", { className: "workagent-muted" }, "DSH 与 Codex / ChatGPT 共享额度。"));
    }
    ...
    function RuntimeConversation({
      ...
      return h(
        ...
        forkTarget ? h("dialog", {
          ...
          onClick: (event) => { if (event.target === event.currentTarget) setForkTarget(null); },
        }, h("div", { className: "workagent-fork-content" },
          h(Icon, { name: "branch", size: 28 }),
          h("h2", { id: "workagent-fork-heading" }, "从这里创建分支？"),
          h("p", null, "将保留到这条消息为止的上下文，在新对话中继续探索。当前对话会保留。"),
          h("div", { className: "workagent-actions" },
            h(Button, { autoFocus: true, onClick: () => setForkTarget(null) }, "继续当前对话"),
            h(Button, { className: "workagent-button is-primary", onClick: () => { const target = forkTarget; setForkTarget(null); void fork(target.messageId, target.replacementContent, true); } }, "创建分支")))) : null,
        side
          ? sideHeader
          : h(
              "header",
