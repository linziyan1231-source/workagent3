...
import { isDarkTheme, watchTheme } from "../../host/compatibility.js";
import {
  createFileComposer,
  bindComposerFiles,
...
export function createWorkbench({
  ...
  apiRoot,
  fileURL,
  workspaceEndpoint = (id) => `${apiRoot}/workspaces/${encodeURIComponent(id)}`,
  nativeSessionAction,
  uploadFiles,
  Icon,
  useUploadToProject = () => true,
}) {
  ...
  const FileComposer = createFileComposer({
    ...
    openFile: (reference) =>
      openFile(reference).catch((error) =>
        window.alert(friendlyError(error.message)),
      ),
  });
  ...
  function Markdown({ children, streaming = false, workspaceId }) {
    ...
    React.useLayoutEffect(() => {
      // The upstream Markdown link renderer drops optional Markdown titles.
      for (const anchor of root.current.querySelectorAll("a[href]")) {
        const url = new URL(anchor.href, location.origin);
        if (
          url.origin === location.origin &&
          /^\/api\/runtime\/v1\/workspaces\/[^/]+\/content$/.test(url.pathname)
        )
      ...
      .map((chunk, index) => {
        ...
        return fileReferenceParts(chunk)
          .map((part) => {
            ...
            const title = file.path.replace(/\\/g, "\\\\").replace(/"/g, '\\"');
            const href = new URL(
              fileURL(
  ...
  function Artifacts({ sessionId, workspaceId, revision }) {
    ...
    React.useEffect(() => {
      ...
      request(
        `${workspaceEndpoint(workspaceId)}/assets?sessionId=${encodeURIComponent(sessionId)}`,
    ...
    }, [sessionId, workspaceId, revision, fileRevision]);
    return rows.length
      ? h(
          "details",
          ...
          ...rows.map((row) =>
            h(
              ...
              h(
                "a",
                {
                  href: fileURL(workspaceId, row.path, false, row.fileId, true),
                  download: row.name,
                },
                "下载",
              ),
  ...
  function Question({
    ...
    const [reply, setReply] = useDraft(`${sessionId}:question:${id}`);
    ...
    const submit = async (event) => {
      ...
      try {
        ...
        setStatus(friendlyError(cause.message));
      } finally {
    ...
    return h(
      ...
      h(
        ...
        h(
          ...
          h("path", { d: "M10 8.5a2 2 0 0 1 4 .5c0 1.3-2 1.5-2 3M12 15h.01" }),
        ),
        h("span", { id: heading }, "补充问题"),
      ),
      ...React.Children.toArray(children),
      onReply &&
        h(
  ...
  function SessionReminder({ sessionId, onSaved }) {
    ...
    React.useEffect(() => {
      ...
      request(`${apiRoot}/completion-notifications`, {
        ...
        .then((value) => {
          if (!controller.signal.aborted) {
            setState(value);
            const configured = value.sessionSettings?.[sessionId];
            ...
            setDraft({
              enabled:
                ...
                (value.enabled === true &&
                  !value.mutedSessions?.includes(sessionId)),
              targetId:
                configured?.targetId ||
                value.lastTargetId ||
                value.targetId ||
    ...
    return h(
      ...
      h(
        ...
        h(
          ...
          Icon ? h(Icon, { name: "notifications", size: 14 }) : null,
          "提醒渠道",
        ),
        h(
          "select",
  ...
  function ComposerTools({
    ...
    const uploadToProject = useUploadToProject();
    ...
    const insert = async (path, savedEntry) => {
      ...
      if (mention === undefined && editor?.workagentInsertReference)
  ...
  function TextEditor({
    ...
    const leaseOwner = React.useRef(crypto.randomUUID());
    React.useEffect(() => {
      if (!dirty) return;
      ...
      const hold = () =>
        ...
        }).catch((error) => setError(friendlyError(error.message)));
      void hold();
      const timer = setInterval(hold, 15000);
      return () => {
        clearInterval(timer);
        void request(url, {
          method: "POST",
          body: JSON.stringify({ action: "lease", owner: leaseOwner.current }),
        }).catch(() => {});
      };
      ...
      h(
        ...
        button(
          ...
          async () => {
            ...
            setError("");
            try {
              await request(fileURL(workspaceId, path, false, fileId), {
                method: "PATCH",
