export function createAutomations({
  ...
  function SkillSuggestion({ row, run, saved }) {
    ...
    return h("section", { className: "workagent-skill-suggestion", "aria-label": "可复用技能建议" },
      h("strong", null, "本次执行生成了技能建议"),
      h(Button, { disabled: busy, onClick: async () => {
        setBusy(true); setError("");
        try { setText(await request(`${apiRoot}/workspaces/${encodeURIComponent(run.definitionSnapshot.workspaceId)}/content?path=${encodeURIComponent(run.skillSuggestionPath)}`)); }
        catch (reason) { setError(friendlyError(reason.message)); }
        finally { setBusy(false); }
      } }, "预览技能建议"),
      h(Button, { disabled: busy, onClick: () => setIgnored(true) }, "忽略建议"),
      text !== null ? h("form", { className: "workagent-form", onSubmit: async (event) => {
        event.preventDefault(); setBusy(true); setError("");
        try {
          let skillId = installedId;
          if (!skillId) {
            const data = new FormData(); data.set("name", name); data.set("description", `来自定时任务 ${row.name} 的执行流程`); data.set("format", "directory"); data.set("paths", JSON.stringify(["SKILL.md"])); data.append("files", new File([text], "SKILL.md", { type: "text/markdown" }));
            const response = await fetch(`${apiRoot}/imports/skill`, { method: "POST", credentials: "same-origin", body: data });
            const result = await response.json();
            if (!response.ok || result.error || !result.resourceId) throw new Error(result.error || "技能导入失败");
            skillId = result.resourceId; setInstalledId(skillId);
          }
          await request(`${endpoint}/${encodeURIComponent(row.id)}`, { method: "PATCH", body: JSON.stringify({ version: row.version, skillId }) });
          saved();
        } catch (reason) { setError(friendlyError(reason.message)); }
        finally { setBusy(false); }
      } },
        h("label", null, "建议技能名称", h(Input, { "aria-label": "建议技能名称", value: name, onChange: (event) => setName(event.target.value), required: true, maxLength: 120 })),
        h("label", null, "建议技能内容", h("textarea", { "aria-label": "建议技能内容", value: text, onChange: (event) => setText(event.target.value), rows: 16, maxLength: 128 * 1024 })),
        h("p", null, installedId ? "技能已保存；若任务版本冲突，请刷新任务后重新绑定。" : "请检查适用范围和执行步骤。保存后，下次运行会使用此技能。"),
        h(Button, { type: "submit", disabled: busy }, "保存技能并绑定任务")) : null,
      error ? h("p", { role: "alert" }, error) : null);
  }
  function Editor({ row, presets, skills, workspaces, sessions, saved, cancel }) {
    ...
    return h(
      ...
      field("绑定技能", h(Select, { name: "skillId", defaultValue: row?.skillId || "", options: [["", "不绑定，执行后可生成建议"], ...skills.filter((skill) => skill.enabled || skill.id === row?.skillId).map((skill) => [skill.id, skill.name])] })),
      field(
        "任务内容",
        h("textarea", {
          name: "input",
      ...
      h(
        ...
        h(
          Button,
          { type: "submit", disabled: busy, className: "workagent-button workagent-automation-create" },
  ...
  function Page() {
    ...
    return h(
      ...
      editing !== null
        ...
        : h(Button, { className: "workagent-button workagent-automation-create workagent-automation-new", onClick: () => setEditing(undefined) }, "新建定时任务"),
      h(Status, { state }),
      error
        ? h("p", { role: "alert", className: "workagent-error" }, error)
        : null,
        ...
        h(
          Card,
          {
            ...
            detail: h(React.Fragment, null,
              h("span", { className: "workagent-automation-state" }, row.enabled ? "已启用" : "已暂停"),
              h("span", null, row.nextRunAt ? `下次运行 ${new Date(row.nextRunAt).toLocaleString()}` : "暂无下次运行时间")),
          },
          h(
            Button,
          ...
          h(Button, { className: "workagent-button workagent-automation-history", onClick: () => loadHistory(row.id) }, "运行记录"),
          h(
            Button,
            {
              disabled: busy,
                  ...
                  ? history[row.id].map((run) =>
                      h(
                        ...
                        run.result
                          ...
                          : null,
                        run.skillSuggestionPath ? h(SkillSuggestion, { row, run, saved: refresh }) : null,
