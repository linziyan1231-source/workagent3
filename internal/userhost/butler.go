...
var butlerRoutes = map[string]*regexp.Regexp{
	...
	"POST":   regexp.MustCompile(`^(/v1/(presets|mcp-servers|credentials|imports/mcp|capability-sync/run|completion-notifications/send)|/v1/mcp-servers/[A-Za-z0-9_:@.-]+/test|/v1/presets/[A-Za-z0-9_:@.-]+/copy|/v1/sessions/[A-Za-z0-9_:@.-]+/capabilities/reload|/dsh-im-connect/api/assistant|/dsh-im-connect/api/accounts/[A-Za-z0-9_:@.-]+/(settings|receive|reconnect|check|remove|approve|deny)|/dsh-im-connect/api/channels/[A-Za-z0-9_:@.-]+/(connect|receive|disconnect|remove)|/dsh-im-connect/api/channels/[A-Za-z0-9_:@.-]+/qr/(start|refresh|cancel))$`),
